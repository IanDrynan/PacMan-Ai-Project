    ...
    for dist in livingGhostPositionDistributions:
        ...
        for d in dist:
            if dist[d] > p:
                likelyPos = d
                p = dist[d]
        currDist = self.distancer.getDistance(pacmanPosition, likelyPos)
        if currDist < closestGhostDist:
            closestGhostDist = currDist
            closestGhostPos = likelyPos

    minDist = closestGhostDist
    bestMove = None
    
    for action in legal:
        nextPos = Actions.getSuccessor(pacmanPosition, action)
        nextDist = self.distancer.getDistance(nextPos, closestGhostPos)
        if nextDist < minDist:
            minDist = nextDist
            bestMove = action
        elif nextDist == minDist:
